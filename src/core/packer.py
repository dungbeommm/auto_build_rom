...
def build_rom_filename_prefix(ctx: Any) -> str:
    ...
    if getattr(ctx, "is_eu_rom", False):
        return "xiaomi.eu_"
    return ""
...
def build_rom_filename_device_tag(ctx: Any) -> str:
    ...
    region = str(getattr(ctx, "global_region", "") or "").lower().strip()
    if region and region != "global":
        return f"{device}_{region}_global"

    if getattr(ctx, "is_global_rom", False):
...
class Repacker:
    def __init__(self, context: Any):
        """
        :param context: RomContext object containing target_dir and other info
        """
        self.ctx = context
        ...
        self.product_out: Path = self.out_dir / "target" / "product" / self.ctx.rom_code
    ...
    def _generate_flash_script(self, super_image_path: Path) -> None:
        ...
        out_name: str = f"{self.ctx.rom_code}_{self.ctx.target_rom_version}_hybrid"
        ...
        final_zip_name: str = (
            f"{self.ctx.rom_code}-hybrid-{self.ctx.target_rom_version}-{timestamp}.zip"
        )
    ...
    def _process_script_placeholders(self, file_path: Path) -> None:
        ...
        replacements: Dict[str, Any] = {
            "device_code": self.ctx.rom_code,
            "baseversion": self.ctx.android_version,
    ...
    def _get_partition_list(self) -> List[str]:
        ...
        partition_info_path = Path(f"devices/{self.ctx.rom_code}/partition_info.json")
    ...
    def _get_super_size(self) -> int:
        ...
        device_code: str = self.ctx.rom_code.upper()
    ...
    def pack_ota_payload(self) -> None:
        ...
        device_custom_dir: Path = Path(f"devices/{self.ctx.rom_code}")
    ...
    def _collect_stock_avb_profile(self) -> Dict[str, Any]:
        ...
        stock_images_dir = Path("build/rom/images")
    ...
    def _get_partition_info_path(self) -> Path:
        return Path(f"devices/{self.ctx.rom_code}/partition_info.json")
    ...
    def _sync_partition_info_from_stock_avb(self, profile: Dict[str, Any]) -> None:
        ...
        payload.setdefault("device_code", self.ctx.rom_code)
    ...
    def _run_ota_tool(self) -> None:
        ...
        timestamp: str = datetime.now().strftime("%Y%m%d%H%M%S")
        output_zip: Path = self.out_dir / f"{self.ctx.rom_code}-ota_full-{timestamp}.zip"
        ...
        try:
            ...
            final_path: Path = (
                self.out_dir
                / f"{prefix}{device_tag}-ota_full-{self.ctx.target_rom_version}-{self.ctx.security_patch}-{timestamp}-{md5}-{self.ctx.android_version}.zip"
            )
