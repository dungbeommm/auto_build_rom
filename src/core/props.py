...
class PropertyModifier(ModifierPlugin):
    ...
    def _sync_product_build_prop_from_stock(self):
        ...
        """
        stock_prop = self.ctx.rom.extracted_dir / "product" / "etc" / "build.prop"
        ...
        rom_props = self._parse_prop_map(stock_prop)
        target_props = self._parse_prop_map(target_prop)
        if not rom_props or not target_props:
        ...
        for key, stock_value in rom_props.items():
            if key in skip_keys:
                skipped_protected += 1
                continue
    ...
    def _apply_custom_props(self):
        ...
        paths = [
            Path("devices/common"),
            Path(f"devices/{getattr(self.ctx, 'base_chipset_family', 'unknown')}"),
            Path(f"devices/{self.ctx.rom_code}"),
        ]
        valid_paths = [p for p in paths if p.exists()]
    ...
    def _global_codename_replacement(self):
        ...
        replacements = [
            (self.ctx.rom_code, self.ctx.rom_code),
        ]

        # Add product model if available
        rom_model = self.ctx.rom.get_prop("ro.product.model")
        base_model = self.ctx.rom.get_prop("ro.product.model")
        if rom_model and base_model and rom_model != base_model:
            replacements.append((rom_model, base_model))
    ...
    def _reconstruct_props(self):
        """
        Reconstruct critical hardware properties from ROM into ROM.
        """
    ...
    def _update_general_info(self):
        ...
        is_eu = getattr(self.ctx, "is_eu_rom", False)
        is_global = getattr(self.ctx, "is_global_rom", False) and not is_eu
        if is_eu:
        ...
        global_region = (getattr(self.ctx, "global_region", "") or "").strip().lower()
    ...
    def _update_density(self):
        ...
        for _part in ["product", "system"]:
            val = self.ctx.rom.get_prop("ro.sf.lcd_density")
            if val:
                base_density = val
                break
    ...
    def _apply_specific_fixes(self):
        ...
        # --- 2. Millet Fix ---
        millet_ver = self.ctx.rom.get_prop("ro.millet.netlink")
        if not millet_ver:
    ...
    def _optimize_core_affinity(self):
        ...
        android_ver = str(self.ctx.android_version)
