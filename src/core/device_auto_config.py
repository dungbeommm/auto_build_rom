...
class DeviceAutoConfig:
    ...
    def __init__(
        ...
        rom_props: Optional[Dict[str, str]] = None,
    ):
        ...
        Args:
            ...
            rom_props: Optional ROM properties from build.prop
        """
        self.device_code = device_code
        self.payload_info = payload_info
        self.rom_props = rom_props or {}
    ...
    def generate_features_json(self) -> Path:
        ...
        product_name = self.rom_props.get("ro.product.product.name", self.device_code)
        if product_name:
    ...
    def generate_props_json(self) -> Optional[Path]:
        ...
        for key in [
            ...
            if key in self.rom_props:
                build_props[key] = self.rom_props[key]
        ...
        for key in [
            ...
            if key in self.rom_props:
                security_props[key] = self.rom_props[key]
...
def auto_configure_device(
    ...
    auto_config = DeviceAutoConfig(device_code, payload_info, rom_props)
...
def get_or_create_device_config(
    ...
    Args:
        ...
        rom_props: Optional ROM properties
        logger: Optional logger instance
        payload_info: Optional pre-parsed payload info (avoids re-extraction)
    ...
    return auto_configure_device(
        payload_path or Path("."), device_code, rom_props, logger, payload_info
    )
