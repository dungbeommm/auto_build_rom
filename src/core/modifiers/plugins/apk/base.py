...
Uses RomContext's built-in tools and shell runner.
"""
...
class ApkModifierPlugin(ModifierPlugin):
    ...
    def _get_cache_key(self) -> Optional[str]:
        ...
        Uses ROM hash instead of individual APK hash to avoid
        ...
        # Use ROM hash as the base
        try:
            rom_hash = self.ctx.cache_manager._compute_rom_hash(self.ctx.rom.path)
        except (FileNotFoundError, AttributeError):
            return None

        # Combine: ROM hash, APK name, modifier class name, cache version
        # This ensures same ROM + same APK + same modifier = cache hit
