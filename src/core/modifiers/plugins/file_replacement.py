...
class FileReplacementPlugin(ModifierPlugin):
    ...
    def modify(self) -> bool:
        ...
        build_ctx.android_version = int(self.ctx.android_version)
        build_ctx.android_version = int(self.ctx.android_version)
        build_ctx.base_device_code = self.ctx.rom_code
        build_ctx.os_version_incremental = (
            self.ctx.rom.get_prop("ro.mi.os.version.incremental") or ""
        )
        build_ctx.is_eu_rom = getattr(self.ctx, "is_eu_rom", False)

        rom_root = self.ctx.rom.extracted_dir
        ...
        for rule in replacements:
            ...
            try:
                self._handle_rule(rule, rtype, rom_root, target_root)
    ...
    def _handle_rule(self, rule: Dict, rtype: str, rom_root: Path, target_root: Path):
        ...
        if rtype == "unzip_override":
            ...
            self._handle_legacy_replacement(rule, rom_root, target_root)
    ...
    def _handle_legacy_replacement(self, rule: Dict, rom_root: Path, target_root: Path):
        ...
        rule_rom_root = rom_root / search_path
        rule_target_root = target_root / search_path

        if not rule_rom_root.exists():
            return

        for pattern in files:
            sources = []
            if match_mode == "glob":
                sources = list(rule_rom_root.glob(pattern))
            elif match_mode == "recursive":
                sources = list(rule_rom_root.rglob(pattern))
            else:
                exact_file = rule_rom_root / pattern
