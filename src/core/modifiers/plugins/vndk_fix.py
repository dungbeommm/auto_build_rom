...
class VNDKFixPlugin(ModifierPlugin):
    ...
    def _fix_vndk_apex(self):
        ...
        vndk_version = self.ctx.rom.get_prop("ro.vndk.version")

        if not vndk_version:
            for prop in (self.ctx.rom.extracted_dir / "vendor").rglob("*.prop"):
                try:
        ...
        stock_apex = self._find_file_recursive(
            self.ctx.rom.extracted_dir / "system_ext/apex", apex_name
        )
