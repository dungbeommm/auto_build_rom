...
Supports two modes:
1. Direct extraction from CN ROM (when stock is CN)
...
class EULocalizationPlugin(ModifierPlugin):
    ...
    def check_prerequisites(self) -> bool:
        ...
        if not getattr(self.ctx, "is_eu_rom", False):
            return False
    ...
    def _is_stock_cn(self) -> bool:
        """Detect if ROM is a CN (China) ROM.
        ...
        """
        if not hasattr(self.ctx, "stock") or not self.ctx.rom:
            return False

        rom_props = self.ctx.rom.props

        # Method 1: Check mod_device (most reliable)
        mod_device = rom_props.get("ro.product.mod_device", "").lower()
        if mod_device:
        ...
        region = rom_props.get("ro.miui.build.region", "").lower()
        if region == "cn":
            return True

        # Method 3: Check product locale
        locale = rom_props.get("ro.product.locale", "").lower()
    ...
    def modify(self) -> bool:
        ...
        if self._is_stock_cn():
            self.logger.info("CN ROM detected, extracting EU apps directly...")
    ...
    def _extract_from_stock(self) -> bool:
        """Extract EU apps directly from CN ROM."""
        ...
        for item in apps_list:
            ...
            # Method 2: If only path provided, find pkg_name from stock first
                ...
                rom_path = self.ctx.rom.extracted_dir / app_path_str
                if rom_path.is_dir():
                    # Find any APK in the directory
                    for apk_file in rom_path.rglob("*.apk"):
                        ...
                        if stock_pkg:
                            ...
                            if target_apks:
                                ...
                                self._remove_target_apks(target_apks)
                            break
                elif rom_path.exists() and rom_path.suffix == ".apk":
                    stock_pkg = self.ctx.syncer._get_apk_package_name(rom_path)
                    if stock_pkg:
                        target_apks = self.ctx.syncer.find_apks_by_package(
                            stock_pkg, self.ctx.target_dir
                        )
        ...
        for item in apps_list:
            ...
            if pkg_name:
                matches = self.ctx.syncer.find_apks_by_package(
                    pkg_name, self.ctx.rom.extracted_dir
                )
                if matches:
            ...
            if not found_srcs and app_path_str:
                ...
                if parts:
                    ...
                    candidates = [
                        self.ctx.rom.extracted_dir / app_path_str,
                        self.ctx.rom.extracted_dir / partition / partition / relative_path,
                    ]
            ...
            for src in found_srcs:
                ...
                try:
                    rel_to_extracted = src.relative_to(self.ctx.rom.extracted_dir)
                except ValueError:
