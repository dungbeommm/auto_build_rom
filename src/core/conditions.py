...
class BuildContext:
    ...
    def __init__(self):
        # ROM type flags
        self.is_eu_rom: bool = False
        self.is_global_rom: bool = False

        # Version information
        self.android_version: int = 0
        self.android_version: int = 0
        self.rom_version: str = ""
        self.os_version_incremental: str = ""
    ...
    @property
    def isColorOS(self) -> bool:
        """Backward-compatible alias for legacy rule schemas."""
        return bool(self.is_eu_rom)

    @property
    def isColorOSGlobal(self) -> bool:
        """Backward-compatible alias for legacy rule schemas."""
        return bool(self.is_global_rom)

    @property
    def isOOS(self) -> bool:
        """Backward-compatible alias for legacy rule schemas."""
        return not bool(self.is_eu_rom or self.is_global_rom)

    @property
    def oplusrom_version(self) -> str:
        """Backward-compatible alias for legacy rule schemas."""
...
class SimpleConditionStrategy(ConditionStrategy):
    ...
    def check(self, rule: Dict[str, Any], ctx: BuildContext) -> bool:
        # ROM Type Conditions
        if rule.get("condition_is_eu_rom") and not ctx.is_eu_rom:
            return False

        # Port Android Version (exact match)
        cond_port_v = rule.get("condition_android_version")
        if cond_port_v is not None and int(ctx.android_version) != cond_port_v:
            return False

        # Base Android Version Less Than
        cond_base_lt = rule.get("condition_android_version_lt")
        if cond_base_lt is not None and int(ctx.android_version) >= cond_base_lt:
            return False

        # Base Android Version Greater Than or Equal
        cond_base_gte = rule.get("condition_android_version_gte")
        if cond_base_gte is not None and int(ctx.android_version) < cond_base_gte:
            return False
        ...
        cond_port_os_v_gte = rule.get("condition_os_version_incremental_gte")
        if cond_port_os_v_gte is not None and ctx.os_version_incremental:
            try:
                # Remove common prefix like 'OS' for parsing if present
                v1 = str(ctx.os_version_incremental).replace("OS", "", 1)
                v2 = str(cond_port_os_v_gte).replace("OS", "", 1)
                if parse_version(v1) < parse_version(v2):
                    return False
            except Exception:
                # Fallback to string comparison
                if str(ctx.os_version_incremental) < str(cond_port_os_v_gte):
                    return False

        # ROM Version
        cond_port_rom_v = rule.get("condition_rom_version")
        if cond_port_rom_v is not None and cond_port_rom_v not in str(ctx.rom_version):
            return False
...
class CompositeConditionStrategy(ConditionStrategy):
    ...
    def _evaluate_condition(self, condition: Dict[str, Any], ctx: BuildContext) -> bool:
        ...
        if "android_version" in condition or "android_version" in condition:
            av_cond = condition.get("android_version") or condition.get("android_version")
            if isinstance(av_cond, dict):
                min_v = av_cond.get("min")
                max_v = av_cond.get("max")
                current = int(ctx.android_version)
