...
ROM Cache机制是一个分层缓存系统，用于加速多机型ROM移植过程。它允许同一个ROM的处理结果（提取的分区和修改的APK）被多个不同机型的移植任务复用，显著减少重复工作。
...
```bash
python main.py --rom stock.zip  port.zip
```
...
```bash
python main.py --rom stock.zip  port.zip --no-cache
```
...
```bash
# 清除所有缓存
python main.py --rom stock.zip  port.zip --clear-cache
